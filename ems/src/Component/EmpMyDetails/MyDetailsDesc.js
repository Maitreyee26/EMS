...
approvalStatus);
  ...
  useEffect(() => {
    if (userDetails && userDetails.emp_name) {
      ...
      console.log("hi correspondence_address", userDetails.correspondence_address.split(" "));
      setDetails({
        primary: {
          fname: fname || "",
          mname: mname || "",
        ...
        contact: {
          ...
          emgcontactNumber: userDetails.alternate_contact_name||"", // Add emergency contact number logic here if available
        },
  ...
  const updateUserDetails = async (updatedDetails) => {
    try {
      const response = await fetch(
        `https://ems-backend-production-3f3d.up.railway.app/editEmployee/${empId}`,
        {
          method: "PUT",
          headers: {
    ...
    const fetchFileData = async () => {
      try {
        ...
        const fetchStatus = async (fileId) => {
          try {
            const response = await fetch(`https://ems-backend-production-3f3d.up.railway.app/findStatusById/${fileId}`);
  ...
  const handleDeleteFile = async (fileId) => {
    try {
      const response = await fetch(`https://ems-backend-production-3f3d.up.railway.app/viewFileById/${fileId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  {isHovered && (
    <span className="hover-text">
      primary details or contact details or address details may be changed
    </span>
  )}
</span>
  ...
  <li key={index}>
    <div className="file-container">
      <div className="file-details">
        ...
        <a
          href={`https://ems-backend-production-3f3d.up.railway.app/viewFileById/${fileId}`}
          target="_blank"
